...
class DDT(nn.Module):
    ...
    def init_comparators(self, comparators):
        ...
        new_comps = torch.tensor(np.array(comparators), requires_grad=True)
        if self.use_gpu:
    ...
    def init_weights(self, weights):
        ...
        new_weights = torch.tensor(np.array(weights), requires_grad=True)
        if self.use_gpu:
    ...
    def init_leaves(self):
        ...
        labels = torch.tensor(np.array(new_leaves))
        if self.use_gpu:
            labels = labels.cuda()
